...
const DashboardPage = () => {
  ...
  useEffect(() => {
    if (!authLoading && user && !fetchedRef.current) {
      ...
      // Delay chart rendering slightly to ensure container is measured correctly
      setTimeout(() => setShowCharts(true), 200);
    }
  ...
  if (loading || authLoading) return <div className="flex h-[60vh] items-center justify-center"><div className="h-10 w-10 animate-spin rounded-full border-4 border-primary border-t-transparent"></div></div>;
  ...
  return (
    <div className="space-y-8">
      ...
      {activeTab === 'overview' ? (
                ...
                {showCharts && (
                  <ResponsiveContainer width="100%" aspect={1.7}>
                    <AreaChart data={data}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#16213e', border: '1px solid #2d2d4e', borderRadius: '8px' }} itemStyle={{ color: '#fff' }} />
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between border-b border-surface-border pb-6 last:border-0 last:pb-0">
                  <div className="flex items-center space-x-4">
                    <div className="h-10 w-10 rounded-full bg-surface-input flex items-center justify-center text-primary"><UserCheck size={20} /></div>
                    <div><p className="text-sm font-semibold text-white">System Update</p><p className="text-xs text-gray-400">Everything is up to date.</p></div>
                  </div>
        ...
        <div key="analytics-tab" className="grid grid-cols-1 gap-8 animate-in fade-in duration-500">
          <div className="glass-card p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="p-6 bg-surface-input rounded-xl border border-surface-border">
                ...
                <div className="w-full bg-gray-700 h-2 rounded-full mt-4"><div className="bg-primary h-full rounded-full w-[94%]"></div></div>
              </div>
              <div className="p-6 bg-surface-input rounded-xl border border-surface-border">
                ...
                <div className="w-full bg-gray-700 h-2 rounded-full mt-4"><div className="bg-green-500 h-full rounded-full w-[98%]"></div></div>
              </div>
              <div className="p-6 bg-surface-input rounded-xl border border-surface-border">
                ...
                <div className="w-full bg-gray-700 h-2 rounded-full mt-4"><div className="bg-blue-500 h-full rounded-full w-[80%]"></div></div>
              </div>
            </div>
              ...
              {showCharts && (
                <ResponsiveContainer width="100%" aspect={2.5}>
                  <AreaChart data={data}>
                    ...
                    <YAxis stroke="#6b7280" />
                    <Tooltip contentStyle={{ backgroundColor: '#16213e', border: '1px solid #2d2d4e' }} />
